...
The scripts should generally be executed in the following order:

### Step 1 — Genomic filtering

```text
01_01_VCF_file_filtering_missing_0.9.txt
```

Filter the initial VCF and remove variants with excessive missing data.

### Step 2 — VCF preparation

```text
01_02vcf preparation_landscape.txt
```

Prepare the filtered genomic dataset for downstream analyses.
...
Characterize and visualize population genetic structure.
...
Generate the phylogenetic representation of genetic relationships.
...
Prepare environmental predictors from WorldClim for the sampling locations.

### Step 6 — Landscape genomic analysis

```text
03_02_landscape_analysis.R
```

Perform genotype–environment association analyses and identify candidate variants.
...
Annotate candidate genes and test for enrichment of biological functions.

## Reproducibility

The pipeline should be executed sequentially because later analyses depend on outputs generated in previous steps. File paths, input filenames, software versions, and R package versions should be documented before running the complete workflow.

The main analytical dependency can be summarized as:

**VCF filtering → VCF preparation → population structure/phylogeny → environmental data → landscape genomics → candidate genes → GO enrichment**
...
├── scripts/
│   ├── 01_01_VCF_file_filtering_missing_0.9.txt
│   ├── 01_02vcf_preparation_landscape.txt
│   ├── 02_01_plotting_pop_structure.R
│   ├── 02_02plotting_phylogenetic_tree.R
│   ├── 03_01_readscript_Worldclim.txt
│   ├── 03_02_landscape_analysis.R
